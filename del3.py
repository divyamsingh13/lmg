...
for x,y in zip(test_set['Customer_ID'],test_set['Store_Code']):
	...
	try:
		if((a['State'].values==b['Region'].values) and ((a['Income_Range'].values=='Below 5000' and counts[x]>1) or a['Income_Range'].values!='Below 5000')):			
...
test_set.to_csv("test_set4.csv",index=False)
print(test_set.head())
