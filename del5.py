...
for x,y in zip(test_set['Customer_ID'],test_set['Store_Code']):
	...
	try:
		if((a['State'].values==b['Region'].values) and ((a['Income_Range'].values=='Below 5000' and counts[x]>2) or a['Income_Range'].values!='Below 5000')):
			test_set[test_set['Customer_ID']==x]['Prediction']=1
			test_set.iloc[i1]['Prediction']=1
			# test_set.at(i1,'Prediction')=1
			test_set.set_value(i1,'Prediction',numpy.random.randint(0, high=2))
			i=i+1
			print(i)
...
test_set.to_csv("test_set6.csv",index=False)
print(test_set.head())
