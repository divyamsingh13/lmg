...
for x,y in zip(test_set['Customer_ID'],test_set['Store_Code']):
	...
	if(a['State'].values=='Unspecified' or a['State'].values=='Others'):
		...
		print(a)
	if(b['Region'].values[0] not in e and (a['Loyalty_Status'].values=='Gold' or a['Points'].values>500)):
		for i in e:
...
test_set.to_csv("test_set9.csv",index=False)
print(test_set.head())
